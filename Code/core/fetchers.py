...
class RuleFetcher:
    ...
    def _fetch_http_rules(self, url: str, format: str) -> List[str]:
        ...
        try:
            ...
            # 判断是否为YAML格式
            is_yaml = self._is_yaml_format(format, content_type, url, response.text)
    ...
    def _is_yaml_format(self, format: str, content_type: str, url: str, text: str) -> bool:
        """
        判断是否为YAML格式
        ...
        Returns:
            bool: 是否为YAML格式
        """
        return (format == 'yaml' or 
                'yaml' in content_type or 
                url.endswith(('.yml', '.yaml')) or 
                text.strip().startswith('payload:'))
    ...
    def _read_local_rules(self, path: str, format: str) -> List[str]:
        ...
        try:
            with open(path, 'r', encoding='utf-8') as f:
                ...
                if self._is_local_yaml_format(format, path):
    ...
    def _is_local_yaml_format(self, format: str, path: str) -> bool:
        """
        判断本地文件是否为YAML格式
        ...
        Returns:
            bool: 是否为YAML格式
        """
        return format == 'yaml' or path.endswith(('.yml', '.yaml'))
