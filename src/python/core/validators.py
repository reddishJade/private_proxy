...
class RuleValidator:
    ...
    def _handle_simple_rule(self, rule: str) -> Optional[str]:
        ...
        # 尝试识别为域名后缀（+.格式）
        if rule.startswith('+.') and self._is_valid_domain(rule[2:]):
            return f"DOMAIN-SUFFIX,{rule[2:]}"
        
        # 尝试识别为IP CIDR
        if (re.match(self.constants.IPV4_CIDR_PATTERN, rule) or 
            re.match(self.constants.IPV6_CIDR_PATTERN, rule)):
            return f"IP-CIDR6,{rule}" if ':' in rule else f"IP-CIDR,{rule}"
        
        return None

    def _validate_rule_by_type(self, rule_type: str, value: str, original_rule: str) -> Optional[str]:
        """
        根据规则类型验证值
        ...
        if rule_type in {'DOMAIN', 'DOMAIN-SUFFIX'}:
            return original_rule if self._is_valid_domain(value) else None
        elif rule_type == 'IP-CIDR':
            return original_rule if re.match(self.constants.IPV4_CIDR_PATTERN, value) else None
        elif rule_type == 'IP-CIDR6':
            return original_rule if re.match(self.constants.IPV6_CIDR_PATTERN, value) else None
        else:
            # 对于其他类型的规则，进行基本验证
